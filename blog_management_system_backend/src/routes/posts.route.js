...
router.get('/my', auth(...roles), (req, res, next) => {
  req.my = true;
  next()
}, validate(postsValidation.getPosts), postsController.getPosts)
...
router.patch('/:postId/:status', auth(...roles), validate(postsValidation.publishUnpublishPost), postsController.publishUnpublishPost)
